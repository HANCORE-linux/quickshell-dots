//
// Rise V2 bar lifecycle: bind one bar to each real Wayland output, skip
...
Item {
    ...
    // IPC handlers must live outside the per-monitor BarSlot delegate.
    // Multi-monitor setups would register the same target once per bar otherwise.
    IpcHandler {
        target: "layout"
    ...
    // exists; exclude it so no unusable layer surface is created.
